...
class Notifications {
    ...
    public function notify_appointment_saved($appointment, $service, $provider, $customer, $settings, $manage_mode = FALSE)
    {
        try
        {
            ...
            $send_all_providers = filter_var(
                $this->CI->settings_model->get_setting('provider_notification'),
                FILTER_VALIDATE_BOOLEAN);
            ...
            if ($send_provider === TRUE and $send_all_providers === TRUE)
            {
                $email->send_appointment_details($appointment, $provider,
            ...
            $send_admin = filter_var(
                $this->CI->settings_model->get_setting('admin_notifications'),
                FILTER_VALIDATE_BOOLEAN);
            ...
            $send_secretary = filter_var(
                $this->CI->settings_model->get_setting('secretary_notifications'),
                FILTER_VALIDATE_BOOLEAN);

            if ($send_secretary === TRUE)
            {
                // Notify secretaries
    ...
    public function notify_appointment_reminder($appointment, $service, $provider, $customer, $settings)
    {
        try
        {
            $email = new EmailClient($this->CI, $this->CI->config->config);
            $customer_title = new Text($settings['company_name'] . ': ' . lang('reminder') . ' ' . lang('appointment'));
            $customer_message = new Text(lang('thank_you_for_appointment'));
